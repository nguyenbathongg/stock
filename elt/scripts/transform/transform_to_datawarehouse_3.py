...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        import os
        data_dirs = [
            '/home/thong/WorkSpace/Project/elt/data/completed',
            '/home/thong/WorkSpace/Project/elt/data/raw'
        ]
        parquet_files = []
        
        # Search for parquet files in the data directories
        ...
        if parquet_files:
            # Prefer news parquet files over others
            news_files = [f for f in parquet_files if 'news' in f.lower()]
            selected_file = news_files[0] if news_files else parquet_files[0]
            print(f"No parquet file path provided. Using: {selected_file}")
            parquet_file_path = selected_file
        else:
            print("Error: No parquet file path provided and no parquet files found in data directory.")
            print("Usage: python transform_to_datawarehouse_3.py <parquet_file_path>")
            sys.exit(1)
    else:
